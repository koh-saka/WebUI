...
function Registration() {
    ...
    function build() {
        if (!buildFlag) {
            ...
            reloadWindow();
            sessionStorage.isWarningPage = false;

            window.addEventListener("orientationchange", function () {
                if (sessionStorage.isWarningPage == 'true') {
                    // alert('111');
                    reloadWindow()
                } else {
                    // alert('222');
    ...
    function reloadWindow() {
        document.activeElement.blur();
        Size = getSize ();
        LoadSize('registrationDiv');
        LoadSize('adminRegistration');
        LoadSize('registrationWarning');

        setTimeout(function () {
            var divWidth = $("#registrationDiv").width();
            var divHeight = $("#registrationDiv").height();
            windowsZoomControl(divWidth, divHeight);
            var isHeight = ($(window).height() - $("#registrationDiv").height() * currentZoomValue) / 2;
            var isWidth = ($(window).width() - $("#registrationDiv").width() * currentZoomValue) / 2;
            var isTop = isHeight;
            var isLeft = isWidth;
            if (isTop < 0) {
                isTop = 0;
            ...
            $("#registrationDiv").css({
                "top": isTop / currentZoomValue + "px",
                "left": isLeft / currentZoomValue + "px"
            });
            setTimeout(function () {
    ...
    function callbackRegUser(mouse) {
        if (mouse == Button.MOUSE_DOWN) {
            ...
            if (errMsg) {
                ...
                sessionStorage.isWarningPage = true;
